function getHistoricalData() {
    ...
    function getData() {
        ...
        // first get metadata on the stock
        ticker = $(".get-data input").val().toUpperCase();
        let $newMeta = $("<p>");
        if (ticker !== "") {
        fetchQuandlMeta(ticker).then(response => {
        newMeta = response;
    },
    error => {
        ...
        let $quote = $("<p>");
        if (ticker !== "") {
        fetchQuandlQuote(ticker).then(response => {
        quote = response;
    },
    error => {
                $(".render-quote").empty().append($quote.html(error));
...
d3.select('#test-container3').selectAll('p')
    ...
    function fetchQuandlMeta(ticker) {
        spinner.show();
        return new Promise((resolve, reject) => {
            let url = `https://www.quandl.com/api/v3/datasets/WIKI/FB/metadata.json?api_key=DTB2cz5Mv7AjBu-kTVZv`;
            // parse JSON
            $.getJSON(url)
                .done(function (json) {
                    if (typeof json !== 'undefined' && typeof json !== 'null' && !json["Error Message"]) {
                        // find necessary quote
                        let key = json["dataset"];
                        console.log("new meta: " + key);
    ...
    function fetchQuandlQuote(ticker) {
        spinner.show();
        return new Promise((resolve, reject) => {
            let url = `https://www.quandl.com/api/v3/datasets/WIKI/${ticker}.json?column_index=4&collapse=daily&&api_key=DTB2cz5Mv7AjBu-kTVZv`;
            // parse JSON
            $.getJSON(url)
                .done(function (json) {
                    if (typeof json !== 'undefined' && typeof json !== 'null' && !json["Error Message"]) {
                ...
                //        let quote = Number(json["Time Series (60min)"][key]["4. close"]);
                        ...
                        let qResponse = json["dataset"]["data"];
                        console.log("some json" + qResponse);
                        spinner.hide();
                        resolve();
                    }
                    else {
